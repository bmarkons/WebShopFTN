adminApp.controller('StoresController', function($scope,$http,$mdDialog,$mdMedia,$mdToast,$rootScope) {
		...
		$scope.showDialog = function(ev,type,store) {
		    ...
		    $mdDialog.show({
		      ...
		      templateUrl: '/WebShop/adminPartials/newStoreDialog.html',
	...
	addStoreDialogController = function($scope, $mdDialog, $http, $rootScope, Countries) {
...
//		});
	...
	editStoreDialogController = function($scope, $mdDialog, $http, $rootScope, Countries) {
		$scope.getAllSellers = function(){
			$http.get("/WebShop/rest/user/sellers").success( function(response) {
				$scope.sellers = response;
			});
